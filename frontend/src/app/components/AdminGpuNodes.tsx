...
import { ArrowLeft, Copy, Cpu, KeyRound, LoaderCircle, Plus, PowerOff, RefreshCw, ShieldCheck, X } from "lucide-react";
...
export default function AdminGpuNodes() {
  ...
  return <main style={pageStyle}>
    <header style={headerStyle}>
      <div>
        <button style={backButton} onClick={() => nav("/admin/templates")}><ArrowLeft size={15} /> 模板工作台</button>
        <h1 style={{ margin: "9px 0 0", fontSize: 27, letterSpacing: 0 }}>GPU 节点</h1>
      </div>
    ...
    <section style={cardStyle}>
      ...
      {loading ? <div style={emptyStyle}><LoaderCircle className="gpu-spin" size={18} /> 正在读取节点</div> : nodes.length === 0 ? <div style={emptyStyle}><Cpu size={22} /> 尚未创建节点。创建后会得到一份仅显示一次的安装配置。</div> : <div style={{ overflowX: "auto" }}>
        <table style={tableStyle}>
          <thead><tr><th>节点</th><th>状态</th><th>能力</th><th>任务</th><th>工作方式</th><th>Agent 模型</th><th /></tr></thead>
          <tbody>{nodes.map((node) => <tr key={node.id}>
            ...
            <td><label style={compactLabel}>下发模型<select value={node.desired_config.claude_model_id ?? ""} disabled={!node.active || busyId === node.id || agentModels.length === 0} onChange={(event) => void run(node.id, async () => { const updated = await updateGpuNodeConfig(node.id, node.desired_config.paused, node.desired_config.max_iterations, event.target.value || null); setNodes((current) => current.map((item) => item.id === node.id ? updated : item)); setNotice("模型配置已下发；节点将在下一次心跳应用"); })}><option value="">节点本地默认</option>{agentModels.map((model) => <option key={model.id} value={model.id}>{model.label}</option>)}</select></label><small style={subtleStyle}>{agentModels.length === 0 ? "暂无兼容模型" : node.desired_config.claude_model || "不覆盖本地设置"}</small></td>
...
const backButton: React.CSSProperties = { display: "inline-flex", alignItems: "center", gap: 5, padding: 0, border: 0, background: "transparent", color: U.textMid, cursor: "pointer", font: "inherit", fontSize: 12 };
